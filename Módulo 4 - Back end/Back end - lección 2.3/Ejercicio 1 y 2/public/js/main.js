...
document.querySelector('.js-btn-get-users').addEventListener('click', () => {
	const filterName = document.querySelector('.js-filtername');
	const filterEmail = document.querySelector('.js-filteremail');
	fetch(
		`http://localhost:3000/users?filterName=${filterName.value}&filterEmail=${filterEmail.value}`
	)
		.then((response) => response.json())
		.then((responseData) => {
			console.log('Server response:', responseData);
